...
export const useCompoundInterestCalculator = (data: CalculatorData): CalculationResult => {
  return useMemo(() => {
    ...
    const futureValue = calculateCompoundInterest(principal, rate, frequency, years);
    const totalInterest = futureValue - principal;
    const growthPercentage = principal > 0 ? ((futureValue / principal) - 1) * 100 : 0;
    const yearlyBreakdown = generateYearlyBreakdown(principal, rate, frequency, years);
