...
export const ResultCard = ({ result, principal }: ResultCardProps) => {
  ...
  return (
    <View className="bg-gradient-to-br from-blue-600 to-indigo-700 p-8 rounded-3xl shadow-xl items-center border border-blue-500">
      <Text className="text-blue-200 text-xs font-bold uppercase tracking-widest mb-3">Your Future Value</Text>
      <Text className="text-white text-5xl font-extrabold mb-6">{formatCurrency(result.futureValue)}</Text>

      <View className="bg-blue-500 bg-opacity-60 px-6 py-3 rounded-full border border-blue-400 w-full">
        <Text className="text-white font-semibold text-center">
          Interest Earned: <Text className="text-blue-100">{formatCurrency(result.totalInterest)}</Text>
        </Text>
      </View>
      ...
      >
        <Text className="text-blue-100 font-semibold">Year-by-Year Breakdown</Text>
        {showBreakdown ? (
      ...
      {showBreakdown && (
        <ScrollView className="mt-4 w-full max-h-64 bg-blue-900 bg-opacity-30 rounded-lg p-4 border border-blue-400">
          {result.yearlyBreakdown.map((year) => (
            <View key={year.year} className="flex-row justify-between mb-3 pb-3 border-b border-blue-500">
              <View className="flex-1">
                <Text className="text-blue-100 font-semibold">Year {year.year}</Text>
                <Text className="text-blue-200 text-xs">+{formatCurrency(year.interestEarned)}</Text>
              </View>
              <Text className="text-blue-50 font-bold text-right">{formatCurrency(year.value)}</Text>
            </View>
          ))}
        </ScrollView>
      )}
